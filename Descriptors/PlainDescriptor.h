#ifndef PLAIN_DESCRIPTOR_H_
#define PLAIN_DESCRIPTOR_H_
...
public:
    explicit PLAINDescriptor(DataFacadeT *facade) : facade(facade), entered_restricted_area_count(0) {}
    ...
    void Run(const RawRouteData &raw_route, http::Reply &reply) final
    {
        ...
        if (config.geometry)
        {
            JSON::Value route_geometry =
                    description_factory.AppendGeometryString(false);
        ...
        if (INVALID_EDGE_WEIGHT != raw_route.alternative_path_length)
        {
            ...
            if (config.geometry)
            {
                JSON::Value alternate_geometry_string =
                        alternate_description_factory.AppendGeometryString(false);
...
//            json_location_hint_array.values.push_back(hint);
            //ObjectEncoder::EncodeToBase64(raw_route.segment_end_coordinates[i].source_phantom, hint);
            json_location_hint_array.values.push_back(raw_route.segment_end_coordinates[i].source_phantom);
        }
//        ObjectEncoder::EncodeToBase64(raw_route.segment_end_coordinates.back().target_phantom, hint);
//        json_location_hint_array.values.push_back(hint);

        //ObjectEncoder::EncodeToBase64(raw_route.segment_end_coordinates.back().target_phantom, hint);
        json_location_hint_array.values.push_back(raw_route.segment_end_coordinates.back().target_phantom);
